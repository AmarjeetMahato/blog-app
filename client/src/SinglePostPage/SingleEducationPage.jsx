...
import EducationRelated from "../relatedPost/EducationRelated";
...
const SingleEducationPage = () => {
  ...
  useEffect(()=>{
      const fetchSinglePost = async() => {
          const res = await fetch(`/api/auth/education/${id}`)
 ...
 const deletePost = async () => {
     try {
          const res = await axios.delete(`/api/auth/education/${id}`)
          if(res?.status === 200){
              navigate('/education')
          }
     } catch (error) {
  ...
  return (
   ...
   {postDetails && (
      ...
      <div className="" >
          <div>
        <img src={postDetails.image} alt="" className="w-full rounded-[5px] object-cover h-[250px] md:h-[300px]  lg:h-[600px]" />
          </div>
          ...
          <div className="space-y-4">
                    ...
                    <div className="flex items-center mt-4 gap-4">
                           ...
                           {session?.admin?.role === "admin" && <div className="flex items-center gap-2">
                                   <Link to={`/education/update/${id}`} className="cursor-pointer bg-green-400 rounded-full px-2 py-2 text-gray-700"><RiEdit2Fill /></Link>
    ...
    <div className="flex gap-14 py-10">
           ...
           <div className="flex-wipper">
            ...
            <div className="flex-item hidden lg:block">
                     ...
                     <EducationRelated currentPostId={postDetails.id}/>
            </div>
    </div>
      </div>
...
export default SingleEducationPage