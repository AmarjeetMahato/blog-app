...
import FoodRelated from "../relatedPost/FoodRelated";
...
const SingleFoodPage = () => {
  ...
  useEffect(()=>{
      const fetchSinglePost = async() => {
          const res = await fetch(`/api/auth/food/${id}`)
 ...
 const deletePost = async () => {
     try {
          const res = await axios.delete(`/api/auth/food/${id}`)
          if(res?.status === 200){
              navigate('/food')
          }
     } catch (error) {
          ...
          <div className="space-y-4">
                    ...
                    <div className="flex items-center mt-4 gap-4">
                           ...
                           {session?.admin?.role === "admin" && <div className="flex items-center gap-2">
                                   <Link to={`/food/update/${id}`} className="cursor-pointer bg-green-400 rounded-full px-2 py-2 text-gray-700"><RiEdit2Fill /></Link>
    ...
    <div className="flex gap-14 py-10">
           ...
           <div className="flex-wipper">
            ...
            <div className="flex-item hidden lg:block">
                     ...
                     <FoodRelated currentPostId={postDetails.id}/>
            </div>
    </div>
  </>
)}
