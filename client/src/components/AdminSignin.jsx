...
const AdminSignin = () => {
    ...
    const {register, handleSubmit,formState: { errors, isSubmitting }} = useForm({
     ...
     const onsubmit = async(data) => {
       console.log(data);
       try {
            const res = await axios.post(`/api/auth/adminsignin`,{...data})
  ...
  return (
    ...
    <div className="bg-white flex items-center max-w-[1200px]">
             {/* Image Sectiion */}
              ...
              {/* Form */}
              <form action="" className="flex md:py-0 py-16  items-center px-14 justify-center">
          <div className="space-y-4 flex-col">
                <div>
                      <h1 className="text-[30px] font-semibold">Admin Sign In</h1>
                </div>
                <div>
