...
const EducationRelated = ({currentPostId}) => {
 ...
 useEffect(() => {
    const fetchData = async () => {
      ...
      const filter = data.filter((item) => item.category === "education");
      setFilter(filter);
      console.log(filter);
    };
    fetchData();
            ...
            {filter && filter.length > 0 &&  filter.map(item => (
                    ...
                    <div className="mt-2 py-2" key={item.id}>{
                        item.id !== currentPostId && (
                            ...
                            <Link to={`/education/${item.id}`} className=" py-1 border-b text-[13px] border-red-500 w-max">Read More...</Link>
                            </>
                        )
                           }
                     </div>
...
EducationRelated.propTypes = {
    currentPostId: PropTypes.string.isRequired,
  };
  

export default EducationRelated