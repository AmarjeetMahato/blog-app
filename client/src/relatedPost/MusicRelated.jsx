...
const MusicRelated = ({currentPostId}) => {
    ...
    {filter && filter.length > 0 &&  filter.map(item => (
            ...
            <div className="mt-2 py-2" key={item.id}>{
                item.id !== currentPostId && (
                    ...
                    <Link to={`/music/${item.id}`} className=" py-1 border-b text-[13px] border-red-500 w-max">Read More...</Link>
                    </>
                )
                   }
             </div>
...
MusicRelated.propTypes = {
    currentPostId: PropTypes.string.isRequired,
  };
  

export default MusicRelated